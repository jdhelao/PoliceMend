        ...
        if (is_array($objs) and sizeof($objs) > 0 and isset($objs[0]['om_codigo']) and isset($objs[0]['oma_descripcion'])) {
            foreach ($objs as $obj) {
                if (isset($obj['oma_codigo']) and !is_null($obj['om_codigo'])) {
                    ...
                    OrdenMantenimientoActividades::where('oma_codigo',$obj['oma_codigo'])->update(['oma_descripcion' => $obj['oma_descripcion'], 'oma_costo' => $obj['oma_costo'], 'oma_estado' => $obj['oma_estado'], 'updated_by' => ($obj['us_codigo']??$obj['us_edit']??null),]);
