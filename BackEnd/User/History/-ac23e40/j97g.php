...
class OrdenMovilizacionController extends Controller
{
    public function getOrderByPerson ($id, $person) {
        $obj = OrdenMovilizaciones::select([DB::raw("orden_abastecimientos.*, sv.*")])
    ...
    public function getReport ($ini, $end) {
        $obj = OrdenMovilizaciones::select([DB::raw("orden_abastecimientos.*, ve.*, vt.vt_nombre, pe.pe_dni, concat(pe.pe_nombre1, ' ', pe.pe_apellido1, ' ' ) as pe_nombres")])
        ->join('solicitud_vehiculos as sv', 'sv.sv_codigo', '=', 'orden_abastecimientos.sv_codigo')
    ...
    public function update (Request $req) {
        if (OrdenMovilizaciones::find($req->oa_codigo)->exists()) {
            $obj = OrdenMovilizaciones::find($req->oa_codigo);
