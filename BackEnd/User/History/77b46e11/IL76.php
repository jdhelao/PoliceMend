...
class OrdenMantenimientoRepuestoController extends Controller
{
    ...
    public function save(Request $req) {
        $objs = $req->input();
        if (is_array($objs) and sizeof($objs) > 0 and isset($objs[0]['om_codigo']) and isset($objs[0]['oma_cantidad'])) {
            foreach ($objs as $obj) {
                if (isset($obj['omr_codigo']) and !is_null($obj['om_codigo'])) {
                    ...
                    OrdenMantenimientoRepuestos::where('omr_codigo',$obj['omr_codigo'])->update(['omr_cantidad' => $obj['omr_cantidad'], 'omr_costo' => $obj['omr_costo'], 'omr_estado' => $obj['omr_estado'], 'updated_by' => ($obj['us_codigo']??$obj['us_edit']??null),]);
                    ...
                    OrdenMantenimientoRepuestos::create(['om_codigo' => $obj['om_codigo'], 'omr_cantidad' => $obj['omr_cantidad'], 'omr_costo' => $obj['omr_costo'], 'created_by' => ($obj['us_codigo']??$obj['us_edit']??null),]);
                }
            }
