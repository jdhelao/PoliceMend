...
@Component({
  selector: 'app-orden-movilizacion-list',
  templateUrl: './orden-movilizacion-list.component.html',
  styleUrls: ['./orden-movilizacion-list.component.scss']
})
export class OrdenMovilizacionListComponent implements OnInit {
  ...
  async getVehicleRequestList() {
    this.loading = true;
    this.http.get<any>(environment.urlAPI + 'solicitud-vehiculos-aprobadas/3/persona/' + this.user?.pe_codigo).subscribe((data: SolicitudVehicular | any) => {
