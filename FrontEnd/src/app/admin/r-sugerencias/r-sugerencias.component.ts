...
@Component({
  selector: 'app-r-sugerencias',
  templateUrl: './r-sugerencias.component.html',
  styleUrls: ['./r-sugerencias.component.scss']
})
export class RSugerenciasComponent implements OnInit {
  dateIni!: Date;
  dateEnd!: Date;
  dataSource !: RSugerencias[];
  displayedColumns: string[] = ['Cant', 'Tipo', 'Distrito', 'Circuito', 'Subcircuito'];
  ...
  loadReport() {
    ...
    if (navigator.onLine) {
      ...
      this.loading = true;
      this.http.get<any>(environment.urlAPI + 'sugerencias/report/' + formatDate(this.dateIni, 'yyyy-MM-dd', 'en') + '/' + formatDate(this.dateEnd, 'yyyy-MM-dd', 'en')).subscribe((data: any) => {
        console.log(data);
        if (data !== null && data !== undefined
          && data.sugerencias !== null && data.sugerencias !== undefined && data.sugerencias.length > 0
        ) {
