...
@Component({
  selector: 'app-orden-mantenimiento-list',
  templateUrl: './orden-mantenimiento-list.component.html',
  styleUrls: ['./orden-mantenimiento-list.component.scss']
})
export class OrdenMantenimientoListComponent implements OnInit {
  ...
  async getVehicleRequestList() {
    this.loading = true;
    this.http.get<any>(environment.urlAPI + 'solicitud-vehiculos-aprobadas/1/persona/' + this.user?.pe_codigo).subscribe((data: SolicitudVehicular | any) => {
