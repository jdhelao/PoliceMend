...
export class OrdenMantenimientoEditComponent implements OnInit {
  ...
  disableControls() {
    /* Constancia */
    if (!this.isCustodian || this.form.value.om_entrega_condicion.length) {
      this.form.get('om_ingreso_aceptacion')?.disable();
    }
    if (!this.isCustodian || this.form.value.om_documento.length) {
      this.form.get('om_entrega_aceptacion')?.disable();
    }
    if (this.isCustodian || this.form.value.om_progreso == 100) {
      this.form.get('om_documento')?.disable();
    }
