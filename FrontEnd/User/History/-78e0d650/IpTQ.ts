...
export class OrdenMantenimientoEditComponent implements OnInit {
  ...
  saveActivities() {
    if (this.lsActivities.length && this.checkActivities()) {
      if (navigator.onLine) {
        this.submitting = true;
        /*
        console.log(this.lsActivities.filter(object => object.us_codigo !== undefined).map(object => {
          return {...object,en_codigo: this.form.value.en_codigo};
        }));*/

        this.http.patch<any>(environment.urlAPI + 'orden-mantenimiento/actividades', (this.lsActivities.filter(object => object.us_codigo !== undefined).map(object => {
