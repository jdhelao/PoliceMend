...
export class OrdenMantenimientoEditComponent implements OnInit {
  ...
  loadVehicleRequest($id: number) {
    this.loading = true;
    this.http
      .get<any>(environment.urlAPI + 'solicitud-vehiculos/' + $id)
      .pipe(first())
      .subscribe({
        next: (data: SolicitudVehicular | any) => {
          if (
            ...
            data !== undefined &&
            data.sv_codigo !== null &&
            data.sv_codigo !== undefined
          ) {
            this.form.patchValue(data as SolicitudVehicular);
            this.loading = false;
            this.vehicleRequest = data;
            this.isCustodian =
              this.vehicleRequest?.pe_codigo ==
              this.user?.pe_codigo; /* owner of request */
  ...
  disableControls() {
    ...
    if (
      !this.isCustodian ||
      this.form.value.om_entrega_condicion.length ||
      this.form.value.om_ingreso_condicion.length == 0
    ) {
      this.form.get('om_ingreso_aceptacion')?.disable();
    }
    if (
      !this.isCustodian ||
      this.form.value.om_documento.length ||
      this.form.value.om_entrega_condicion.length == 0
    ) {
  ...
  loadActivities($id: number) {
    ...
    this.http
      .get<any>(
        environment.urlAPI + 'orden-mantenimiento/' + $id + '/actividades'
      )
      .pipe(first())
      .subscribe({
  ...
  onSubmit() {
    ...
    if (navigator.onLine) {
      ...
      (this.form.value.om_codigo
        ? this.http.put<any>(
            environment.urlAPI + 'orden-mantenimiento',
            this.form.value
          )
        : this.http.post<any>(
            environment.urlAPI + 'orden-mantenimiento',
