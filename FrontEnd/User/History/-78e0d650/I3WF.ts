...
import { User, SolicitudVehicular, OrdenMantenimiento, OrdenMantenimientoActividad, OrdenMantenimientoRepuesto } from '@app/_models';
...
export class OrdenMantenimientoEditComponent implements OnInit {
  ...
  lsSpareParts: OrdenMantenimientoRepuesto[] = [];
        ...
        .subscribe(
          {
            next: (data: OrdenMantenimiento | any) => {
              if (data !== null && data !== undefined && data.om_codigo !== null && data.om_codigo !== undefined) {
                ...
                this.loading = false;
                this.isFinished = (Number(data.om_progreso) === 100);
                this.loadVehicleRequest(data.sv_codigo);
                this.loadActivities(data.om_codigo);
                this.loadSpareParts(data.om_codigo);
              }
            },
            error: (error) => {
  ...
  editActivity($index: number, $type: number, $detail: any) {
    ...
    this.lsActivities[$index].us_codigo = this.user?.us_codigo; // in order to send only edited elements
    this.sumTotal();
  }
  ...
  async loadSpareParts($order: number = 0) {
    this.loading = true;
    this.lsSpareParts = [];
    this.http.get<any>(environment.urlAPI + 'orden-mantenimiento/' + $order + '/repuestos').subscribe((data: OrdenMantenimientoRepuesto | any) => {
      if (data !== null && data !== undefined && data.length > 0) {
        this.lsSpareParts = data;
        if (this.isCustodian || this.form.value.om_progreso == 100) {
          this.lsSpareParts = (this.lsSpareParts.filter(object => object.omr_estado !== null).map(object => {
            return { ...object };
          }));
        }
      }
  ...
  get filterSpareParts() {
    ...
    fo.key = ['re_nombre'];
    fo.inputValue = this.form.value.searchSparePart;
    return fo;
  }

  clearSearchSpareParts() {
    this.form.patchValue({ searchSparePart: '' });
  }

  editSparePart(re_codigo: number, type: number, detail: any) {
    const index: number = -1;
    this.lsSpareParts.forEach((omr, i) => {
      if (omr.re_codigo == re_codigo) {
        if (type == 1) {
          this.lsSpareParts[i].omr_cantidad = detail.value;
          if (Number(this.lsSpareParts[i].omr_cantidad) > 0) {
            this.lsSpareParts[i].omr_estado = true;
          }
          else {
            this.lsSpareParts[i].omr_cantidad = 0;
            this.lsSpareParts[i].omr_estado = false;
          }
        }
        else if (type == 2) {
          this.lsSpareParts[i].omr_costo = detail.value;
        }
        else if (type == 3) {
          this.lsSpareParts[i].omr_estado = !this.lsSpareParts[i].omr_estado;
          if (this.lsSpareParts[i].omr_estado == true && Number(this.lsSpareParts[i].omr_cantidad) == 0) {
            this.lsSpareParts[i].omr_cantidad = 1;
          }
          else if (this.lsSpareParts[i].omr_estado == false) {
            this.lsSpareParts[i].omr_cantidad = 0;
          }
        }

        this.lsSpareParts[i].us_codigo = this.user?.us_codigo; // in order to send only edited elements
      }
    });
    this.sumTotal();
  }
  ...
  sumTotal() {
    ...
    for (const omr of this.lsSpareParts) {
      if (Number(omr.omr_cantidad) > 0) {
        total += Number(omr.omr_cantidad) * Number(omr.omr_costo);
      }
    }
  ...
  saveSpareParts() {
    let _edited = (this.lsSpareParts.filter(object => object.us_codigo !== undefined).map(object => {
      return { ...object, om_codigo: this.form.value.om_codigo };
    }));

    if (_edited.length && this.checkActivities()) {
      if (navigator.onLine) {
        this.submitting = true;
        this.http.patch<any>(environment.urlAPI + 'orden-mantenimiento/repuestos', _edited)
          .pipe(first())
          .subscribe(
            {
              next: (data: OrdenMantenimientoRepuesto[] | any) => {
                this.submitting = false;
                if (data !== null && data !== undefined && data.length) {
                  this.lsSpareParts = data;
                  //this.router.navigateByUrl('admin/entidad');
