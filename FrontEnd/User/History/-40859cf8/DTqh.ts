...
export class OrdenMovilizacionEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.id) {
      ...
      this.loading = true;
      this.http.get<any>(environment.urlAPI + 'orden-movilizacions/'+this.id+'/persona/' + this.user?.pe_codigo)
        .pipe(first())
        .subscribe(
          {
  ...
  onSubmit() {
    ...
    if (navigator.onLine) {
      ...
      (this.form.value.od_codigo ?
        this.http.put<any>(environment.urlAPI + 'orden-movilizacions', this.form.value) :
        this.http.post<any>(environment.urlAPI + 'orden-movilizacions', this.form.value))
        .pipe(first())
        .subscribe(
          {
            next: (data: OrdenMovilizacion | any) => {
