...
export class RMantenimientosComponent implements OnInit {
  ...
  loadReport() {
    ...
    if (navigator.onLine) {
      ...
      this.loading = true;
      this.http.get<any>(environment.urlAPI + 'orden-mantenimiento/reporte/' + formatDate(this.dateIni, 'yyyy-MM-dd', 'en') + '/' + formatDate(this.dateEnd, 'yyyy-MM-dd', 'en')).subscribe((data: RMantenimiento[]) => {
        console.log(data);
