...
export class RMovilizacionesComponent implements OnInit {
  ...
  loadReport() {
    ...
    if (navigator.onLine) {
      ...
      this.loading = true;
      this.http.get<any>(environment.urlAPI + 'orden-movilizaciones/reporte/' + formatDate(this.dateIni, 'yyyy-MM-dd', 'en') + '/' + formatDate(this.dateEnd, 'yyyy-MM-dd', 'en')).subscribe((data: RAbastecimiento[]) => {
        console.log(data);
        this.loading = false;
...
export interface RAbastecimiento extends SolicitudVehicular, OrdenMovilizacion, Vehiculo, Personal {
