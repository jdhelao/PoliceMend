...
export class RAbastecimientosComponent implements OnInit {
  ...
  loadReport() {
    ...
    if (navigator.onLine) {
      ...
      this.loading = true;
      this.http.get<any>(environment.urlAPI + 'orden-abastecimientos/reporte/' + formatDate(this.dateIni, 'yyyy-MM-dd', 'en') + '/' + formatDate(this.dateEnd, 'yyyy-MM-dd', 'en')).subscribe((data: any) => {
        console.log(data);
